...
/*
 ...
 Используем технику шаблонного наследника от общего шаблонного интерфейста = parent inversion.
...
class Drawable {
    ...
    struct DrawableObject final : IDrawable {
        T m_data{};
        DrawableObject(T x) : m_data(std::move(x)) {}
        ...
        void draw_(Screen &out, size_t position) const override {
            ::draw(m_data, out, position); // for each T should be determined
        }
    };
    ...
    public:
    template <typename T>
    Drawable(T x) : m_self(std::make_unique<DrawableObject<T>>(std::move(x))) {}
